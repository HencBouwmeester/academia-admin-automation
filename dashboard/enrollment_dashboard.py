...
app = dash.Dash(
    ...
    requests_pathname_prefix='/enrollment/',
    routes_pathname_prefix='/enrollment/',
)
...
def build_grouped_replica_pdf(dataframe, group_by_col, term_title, output_target):
    ...
    header_table_data = [
        [Paragraph(term_title + " Schedule", sub_title), Paragraph(right_label, sort_tag)]
    ]
    ...
    for i, (group_name, group_data) in enumerate(grouped):
        for _, row in group_data.iterrows():
            table_matrix.append([
                Paragraph(str(row['CRN']), td_style),
                Paragraph(str(row['Days']), td_style),
                Paragraph(str(row['Time']), td_style),
                Paragraph(str(row['Class']), td_style),
                Paragraph(str(row['Section']), td_style),
                Paragraph(str(int(row['Credit'])), td_style),
                Paragraph(str(row['Title']), td_style),
                Paragraph(str(row['Instructor']), td_style),
                Paragraph(str(row['Bldg']), td_style),
    ...
    html.Div([
        ...
        html.Div([
                ...
                html.Div([
                    ...
                    html.Button('Export PDF (Instructor)', id='export-pdf-instructor-button', n_clicks=0,
                                style={'marginLeft': '5px'}, className='button'),
                    dcc.Download(id='datatable-pdf-instructor-download'),
                    html.Button('Export PDF (Course)', id='export-pdf-course-button', n_clicks=0,
                                style={'marginLeft': '5px'}, className='button'),
...
def export_all(all_n_clicks, dean_n_clicks, data, report_term, term_code):
    if DEBUG:
        print("function: export_all")
    ctx = dash.callback_context
    trigger = ''
    ...
    term_code = convert_term_title_to_code(detect_academic_term(_df))
...
def export_pdf_reports(instructor_clicks, course_clicks, table_data, report_term, term_code):
    ctx = dash.callback_context
    ...
    df['Time'] = df['Time'].fillna('TBA').astype(str)
    df['Bldg'] = df['Bldg'].fillna('ONLI').astype(str)
    ...
    if trigger_id == 'export-pdf-instructor-button' and instructor_clicks > 0:
        ...
        return [{
            'base64': True,
            'content': encoded_pdf,
            'filename': f"Schedule Sort by Instructor {report_term}.pdf",
        ...
        return [dash.no_update, {
            'base64': True,
            'content': encoded_pdf,
            'filename': f"Schedule Sort by Course {report_term}.pdf",
            'type': 'application/pdf'
        }]
